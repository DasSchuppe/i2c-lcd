...
echo "=== QAPASS LCD Service Installer ==="
...
# 2) package.json sauber schreiben
...
class LCM1602 {
  ...
  init() {
    ...
    this.sleep(50); // Wait >40ms after VCC
    this.sendInitCommand(0x30);
    this.sleep(5);
    this.sendInitCommand(0x30);
    this.sleep(1);
    this.sendInitCommand(0x30);
    this.sendInitCommand(0x20); // Set 4-bit mode
  ...
  sendInitCommand(cmd) { this.writeNibble((cmd & 0xF0) | LCD_BACKLIGHT); }
