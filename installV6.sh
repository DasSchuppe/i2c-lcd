...
class LCM1602 {
  ...
  scrollText(text, line=0, speed=500) {
    if(this.scrollInterval) clearInterval(this.scrollInterval);
    text = text.toString().padEnd(this.cols, ' ') + '   ';
    let pos = 0;
    this.scrollInterval = setInterval(() => {
      let displayText = text.substr(pos, this.cols);
      this.print(displayText, line);
      pos++;
      if(pos > text.length - this.cols) pos = 0;
    }, speed);
  }
  ...
  showTitle(title, artist) {
    if(this.scrollInterval) clearInterval(this.scrollInterval);
    this.clear();
    if(artist.length > this.cols) this.scrollText(artist, 0);
    else this.print(artist,0);
    if(title.length > this.cols) this.scrollText(title,1);
    else this.print(title,1);
  }
