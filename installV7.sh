...
class LCM1602 {
  ...
  showTitle(title, artist) {
    ...
    if (title.length > this.cols) {
      this.scrollText(title, 1);
      this.print(artist.slice(0, this.cols), 0);
    } else {
      this.print(artist.slice(0, this.cols), 0);
      this.print(title.slice(0, this.cols), 1);
    }
  }

  scrollText(text, line = 1) {
    let pos = 0;
    const displayWidth = this.cols;
    this.scrollInterval = setInterval(() => {
      const segment = text.substr(pos, displayWidth).padEnd(displayWidth, ' ');
      this.print(segment, line);
...
try {
  ...
  console.error("LCD initialization failed:", err);
}

let welcomeDone = false;
setTimeout(() => { welcomeDone = true; }, 10000);

let socket;
try {
  socket = io('http://localhost:3000', { reconnection: true, timeout: 5000 });
} catch(err) {
...
if(socket) {
  ...
  socket.on('pushState', (state) => {
    ...
    try {
      if(state.status === 'play') lcd.showTitle(state.title||'', state.artist||'');
      else lcd.showTitle('Pause','');
    } catch(err) {
