...
# 4) index.js mit Debug & Welcome 10 Sekunden
...
setTimeout(() => {
    console.log('Switching to song info...');
    // Hole initial state via HTTP
    ...
    }).on('error', err => console.error('HTTP getState error:', err));
}, 10000); // 10 Sekunden

// Socket.io für Updates
try {
    const socket = io('http://localhost:3000', {timeout:5000, reconnection:false});
    socket.on('connect', () => console.log('Connected to Volumio via Socket.io'));

    socket.on('pushState', (state) => {
        console.log('Received pushState:', state);
